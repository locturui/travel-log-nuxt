<script lang="ts" setup generic="T extends PointItem">
import type { FetchError } from "ofetch";
import type { ZodSchema } from "zod";

import { toTypedSchema } from "@vee-validate/zod";

import type { NominatimRes, PointItem } from "~/lib/types";
...
function searchResultSelected(result: NominatimRes) {
...
onMounted(() => {
  mapStore.addedPoint = {
    ...
    name: "Added Point",
    description: "",
    long: props.initialValues?.long || (INNO as [number, number])[0],
    lat: props.initialValues?.lat || (INNO as [number, number])[1],
    zoom: props.zoom,
  };
});
